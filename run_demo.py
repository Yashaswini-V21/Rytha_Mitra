...
def main() -> None:
    ...
    print("  ರೈತ ಗೆಳತಿ · harvest hex harvesters")
    print("═" * 60)
    ...
    print(f"  Land     : {SAMPLE_INPUTS['land_acres']} acres  |  Input Costs : ₹{SAMPLE_INPUTS['input_costs']:,}")
    print()

    groq_key = os.getenv("GROQ_API_KEY", "")
    if groq_key:
        print(f"  ✅  GROQ_API_KEY loaded ({groq_key[:12]}...)")
    else:
        print("  ⚠️   GROQ_API_KEY not set — falling back to heuristic mode")
